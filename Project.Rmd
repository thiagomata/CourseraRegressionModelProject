...
The influence of differents Tramission Types on the MPG is around 0.5417548, from -1.1798384 to 2.263348 in the confidence interval of `r CONFIDENCE_INTERVAL * 100`% . Despide of getting a positive value, when we consider our `r CONFIDENCE_INTERVAL * 100`% confidence interval, there is a reasonable chance that the effect of the different types of Transmission Types is zero. In that sense, it **is inconclusive if some Transmission Type is better for MPG**. But, we still can affirm that, if the Transmission Type affects the MPG, the impact is very small.
...
```{r confit_am_only,results='asis',echo=TRUE,include=FALSE}
confitModelA <- confint(modelAAmOnly,"am",level=CONFIDENCE_INTERVAL)
cat(knitr::kable(confitModelA) %>%
    kable_styling(bootstrap_options = "striped", full_width = F, position = "center"))
```

So, in that sense, the Type of Transmission is a good to predict if that the car presents many features that, all together, increase the car consume. Considering the  `r CONFIDENCE_INTERVAL * 100`% of confidence interval, that effects may vary in the interval of `r confitModelA[1,1]` to `r confitModelA[1,2]`.

```{r confit_strong,results='asis',echo=TRUE,include=FALSE}
...
```{r confit_residual,results='asis',echo=TRUE,include=FALSE}
...
after removing all the influence of the others fields, the influence of the Type of Transmission from Automatic to Manual on the MPG is `r modelResidualOfB$coefficients[["am"]]` with residual squared error of `r getResidualSquaredError(modelResidualOfB)`. Considering the  `r CONFIDENCE_INTERVAL * 100`% of confidence interval, that effects may vary in the interval of `r confitResidual[1,1]` to `r confitResidual[1,2]`.

```{r plot_residuals,echo=TRUE,include=FALSE}
...
As we can see on the residual plot, using just the Transmission Type to try to predict the MPG have a strong RSS and cleary is not a good model. Adding the Transmission Type, on the model reduce the Residual Sum of Squareds in `r setNumberPrecision((100*(rssC - rssB)/rssC), 2)`%.
